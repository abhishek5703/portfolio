...
const About = () => {
  return (
    <section id="about" className="py-20 px-6 bg-white dark:bg-gradient-to-b dark:from-black dark:to-neutral-900">
      <motion.div
      ...
      >
        <h2 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-cyan-600">
          About Me
        </h2>

        <p className="mt-6 text-lg text-gray-700 dark:text-gray-300 leading-relaxed max-w-3xl mx-auto">
          Hey there! I'm <span className="font-bold text-black dark:text-white font-pacifico text-xl">Abhishek Kumar</span>, a 3rd-year B.Tech student at <span className="text-teal-600 dark:text-teal-400 font-semibold">NSUT, Delhi</span> 🇮🇳.
          I’m passionate about building impactful digital experiences and solving real-world problems with clean, efficient code.
        </p>

        <p className="mt-4 text-md text-gray-600 dark:text-gray-400 max-w-3xl mx-auto">
          With a solid foundation in both frontend and backend technologies, I craft full-stack solutions using tools like
          <span className="text-cyan-500 font-semibold"> HTML, CSS, JavaScript, Tailwind, React, Node.js</span> and databases like <span className="text-emerald-500 font-semibold">MongoDB & MySQL</span>. I love working with cloud platforms like
          <span className="text-purple-400 font-semibold"> Vercel</span> and <span className="text-indigo-400 font-semibold">Render</span> to deploy seamless apps.
        </p>

        <h3 className="mt-14 mb-6 text-2xl font-semibold text-gray-800 dark:text-white">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {tech.map((t, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="flex flex-col items-center gap-2 bg-white/30 dark:bg-white/5 backdrop-blur-md p-4 rounded-xl shadow-lg hover:scale-105 transition-transform duration-300 border border-gray-200 dark:border-neutral-800"
            >
              <div>{t.icon}</div>
